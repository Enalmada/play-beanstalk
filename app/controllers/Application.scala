...
class Application @Inject()(implicit dbApi: DBApi, configuration: play.api.Configuration) extends Controller {

  def index = Action {

    // Just something simple to make sure my db is connected
    // If it is connected, then env overrides must be working
    val ids: Seq[String] = if (configuration.getBoolean("db.default.url").nonEmpty) {
      dbApi.database("default").withConnection { implicit connection => SQL"""
        SELECT table_name
    ...
    val mySetting = configuration.getString("my.setting").get
